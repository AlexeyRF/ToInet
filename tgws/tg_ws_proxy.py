...
async def _handle_client(reader, writer, secret: bytes):
    ...
    try:
        ...
        if ws is None and ws_timed_out and proxy_config.fronting_sni:
            log.info("[%s] DC%d%s -> fronting fallback (SNI %s)",
                     label, dc, media_tag, proxy_config.fronting_sni)
            try:
                ws = await RawWebSocket.connect(target, domains[0],
                                                timeout=5.0,
                                                sni=proxy_config.fronting_sni,
                                                path=ws_path)
            except Exception as exc:
                stats.ws_errors += 1
            ...
            if ws:
                ...
                log.info("[%s] DC%d%s fronting OK", label, dc, media_tag)
