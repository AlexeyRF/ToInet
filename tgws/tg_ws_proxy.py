...
async def _read_client_init(reader, writer, secret, label, masking, first_byte_peek=None):
    ...
    if first_byte_peek is not None:
        first_byte = first_byte_peek
    else:
        try:
...
async def _handle_client(reader, writer, secret: bytes):
    ...
    try:
        # Sneak peek at the first byte to determine protocol
        first_byte_peek = await asyncio.wait_for(reader.readexactly(1), timeout=10)
        
        if first_byte_peek == b'\x05':
            # It's a SOCKS5 connection!
            from .socks5_handler import handle_socks5
            await handle_socks5(first_byte_peek, reader, writer, secret)
            return

        # It's MTProto (or FakeTLS), pass the pre-read byte down
        init = await _read_client_init(
            reader, writer, secret, label, proxy_config.fake_tls_domain, first_byte_peek=first_byte_peek)
        if init is None:
            return
